...
(define nmbs%
  (class object%
    ...
    (define/public (route loco-id end-id)
      ...
      (define route-obj (make-object route% railway loco-current-track))
      (define route (send route-obj get-route end))
      (define direction (send loco get-direction))
      (when (eq? (get-field segment (cadr route))
      ...
      (define (reverse-loco route speed)
        (send loco update-location (cadr route))
        (sleep clearance)
        (prepare-next (cdr route))
        (change-loco-direction loco-id)
        (route-iter (cdr route)
                    (- (send (cadr route) get-length) (* clearance speed))
                    speed))
      (define (route-iter route travelled speed)
        (sleep interval)
          ...
          (if db?
            ...
            (let ((db (get-track db?)))
              (cond
                ...
                ((eq? db curr)
                 (route-iter route (+ travelled (* interval speed)) speed))
                ; loco on differne db
                ((reverse? route)
                 (reverse-loco route speed))
                ((eq? db (cadr route))
                 ...
                 (route-iter (cdr route)
                             ;0 ;interval
                             0 ;(* interval (abs speed))
                             speed))
                (else (route-iter route (+ travelled (* interval speed)) speed))))
            (cond ((is-a? curr block%)
                   ; last iteration, loco was on a db, but no longer
                   (send loco update-location (cadr route))
                   (prepare-next (cdr route))
                   (route-iter (cdr route) 0 speed))
                  ((and (> travelled (send (car route) get-length))
                        (not (is-a? (cadr route) block%)))
                   (if (reverse? route)
                     (reverse-loco route speed)
                     (begin (send loco update-location (cadr route))
                            (prepare-next (cdr route))
                            (route-iter (cdr route) 0 speed))))
                  (else
                   (route-iter route
                               (+ travelled (* interval speed))
                               speed))))))
      (define (start)
        (prepare-next route)
        (route-iter route 0 speed))
      (thread start))
...
(define route%
  (class object%
    ...
    (define/public (get-route to)
      (do ((route (list to) (cons (hash-ref how-to-reach (car route)) route)))
        ((eq? (car route) start) route)))
      ))
